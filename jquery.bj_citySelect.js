(function($){
	var city={
		...
		oCity : [{hot:[]},{A:[],B:[]},{C:[],D:[]},{E:[],F:[],G:[]},{H:[]},{J:[]},{K:[],L:[]},{M:[],N:[],P:[]},{Q:[],R:[]},{S:[]},{T:[]},{W:[]},{X:[]},{Y:[]},{Z:[]}],
		//填充html
		fillHtml:function(obj,args){
			return (function(){
				obj.append($("<div class='cityBox' id='cityBox'></div>"))
				if($(args.inputElement).is('input')||$(args.inputElement).is("textarea")){
					$("#cityBox").append($(city.template[0]));
				}
				$("#cityBox").append($(city.template[1]));
				$.each(city.template, function(i,n) {
					if(i>1){
						$("#tab-head").append($(n));
					}
				});
				$("#cityBox").append($("<div class='hotCity'></div>"));
				
				var citys = city.allCity;
				var reg = city.regEx,match,letter,
				regExChiese = /([\u4E00-\u9FA5\uf900-\ufa2d]+)/,
				...
				var regs =[/^[a-b]$/i,/^[c-d]$/i,/^[e-g]$/i,/^[h]$/i,/^[j]$/i,/^[k-l]$/i,/^[m-p]$/i,/^[q-r]$/i,/^[s]$/i,/^[t]$/i,/^[w]$/i,/^[x]$/i,/^[y]$/i,/^[z]$/i]

				//城市按拼音分组
				$.each(citys, function(i,n) {
					match = reg.exec(n);
					letter = match[3].toUpperCase();
					for (var regIndex =0 ;regIndex<regs.length; regIndex++) {
						if (regs[regIndex].test(letter)) {
			                if (!city.oCity[regIndex+1][letter]) city.oCity[regIndex+1][letter] = [];
			                city.oCity[regIndex+1][letter].push(match[1]);
			            }
					}
		            if(i<20){
		                if(!city.oCity[0]['hot']) city.oCity[0]['hot'] = [];
		                city.oCity[0]['hot'].push(match[1]);
		            }
				});
				$.each($("#tab-head li"),function(i,n){
					var dt=Array();
					var citytab,tab;
					if($(n).text()=="热门城市"){
						citytab="<div class='hot cityTab'></div>";
						dt[0] ="&nbsp;";
						tab= "hot";
					}else{
						citytab="<div class='"+$(n).text()+" cityTab hide'></div>";
						for(var index=0;index<$(n).text().length;index++){
							dt[index] = $(n).text()[index];
						}
						tab=$(n).text() ;
					}
					$(".hotCity").append($(citytab));
					var dd="",dl="";
					$.each(dt, function(index,value) {
						var dds =city.oCity[i];
						dd="";
						$.each(dds[value=="&nbsp;"?"hot":value], function(s,b) {
							dd+="<a href='#'>"+b+"</a>";
						});
						dl += "<dl><dt>"+value+"</dt><dd>"+dd+"</dd></dl>";
					});
					$($(".cityTab")[i]).html(dl);
				});
			})();
		...
		//生成提示列表
		dropdown:function(obj,args,value){
			return (function(){
				if($(".cityDown").length<1){
					$(args.inputElement).after($("<ul class='cityDown'></ul>"));
				}else{
					$(".cityDown").html("");
					$(".cityDown").removeClass("hide");
				}
				$(".cityDown").css("top",($(args.inputElement).position().top+$(args.inputElement).outerHeight())+"px").css("left",$(args.inputElement).position().left+"px").css("width",$(args.inputElement).innerWidth());
				if(value!=""){
					var downs=[];
					var reg = new RegExp("^" + value + "|\\|" + value, 'gi');
					$.each(city.allCity, function(i,v) {
						if (reg.test(v)) {
							var match = city.regEx.exec(v);
							downs.push("<li class='cityList'>"+match[1]+"</li>");
						}
					});
					if(downs.length<1){
						downs.push("<li class='noFind'>没有找到<span class='error'>"+value+"</span></li>");
						$(".cityDown").css("overflow","hidden");
					}else{
						$(".cityDown").css("overflow","auto");
					}
					$.each(downs, function(i,v) {
						$(".cityDown").append($(v));
					});
				}
			})();
		...
		bindEvent:function(obj,args){
			return (function(){
			    //切换tab
				obj.on("click","#tab-head li",function(){
					if(!$(this).hasClass("current")){
						var classname = $(this).siblings(".current").text();
						$(this).siblings(".current").removeClass("current");
						$(this).addClass("current");
						$(".hotCity ."+(classname!="热门城市"?classname:"hot")).addClass("hide");
						$(".hotCity ."+($(this).text()!="热门城市"?$(this).text():"hot")).removeClass("hide");
					}
				});
				//点击城市（非提示列表），在指定控件显示选择的城市
				obj.on("click","dl dd a",function(){
					$(args.showElement).is('input')||$(args.showElement).is("textarea")?$(args.showElement).val($(this).text()):$(args.showElement).text($(this).text());
				});
				//在输入控件输入内容时，显示提示列表
				$(args.inputElement).on("input propertychange",function(){
					city.dropdown(obj,args,$(this).val());
				});
				//阻止事件冒泡，防止点击输入控件的时候隐藏提示列表
				$(args.inputElement).on("click",function(){
					 event.stopPropagation();
				});
				//点击输入控件以外区域时隐藏提示列表
				$("body").on("click",function(){
					if($(".cityDown").length>0){
						$(".cityDown").addClass('hide');
					}
				});
				//点击城市（提示列表），在指定控件显示选择的城市
				$("body").on("click",".cityDown .cityList",function(){
					$(args.inputElement).val($(this).text());
				});
			})();
		},
	}
	
	$.fn.bj_citylist=function(options){
		var default_data ={
			inputElement: options.showElement,
			width:"auto"
		},settings;
		settings = $.extend(default_data, options);
		city.init(this,settings);
	}
})(jQuery);